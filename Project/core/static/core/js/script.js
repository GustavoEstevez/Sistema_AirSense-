...
(() => {
  ...
  function isUnhealthyEnv(tState, hState, nState, cState) {
    return [tState, hState, nState, cState].includes('danger');
  }

  // -------- Alerta automática al ingresar al dashboard --------
  async function requestEntryAlertIfNeeded(tState, hState, nState, cState) {
    if (entryAlertRequested) return;
    if (!isUnhealthyEnv(tState, hState, nState, cState)) return;

    entryAlertRequested = true;
    try {
      await fetch('/dashboard/alerta-ingreso/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: '{}',
      });
    } catch (err) {
      console.warn('[AIRSENSE] No se pudo solicitar alerta de ingreso:', err.message);
    }
  }
  ...
  function handleData(data) {
    ...
    requestEntryAlertIfNeeded(tState, hState, nState, cState);
  ...
  // -------- Envío manual de lecturas por correo --------
  function setupEmailForm() {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      if (!email || !email.includes('@')) {
        feedback.className = 'email-feedback is-error';
      ...
      try {
        const res = await fetch('/dashboard/enviar-correo/', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        try {
          ...
          data = { ok: false, mensaje: 'El servidor respondió con un error inesperado.' };
        }
        feedback.className = 'email-feedback ' + (data.ok ? 'is-ok' : 'is-error');
        feedback.textContent = data.mensaje || (data.ok ? 'Correo enviado.' : 'No se pudo enviar.');
      } catch (err) {
        feedback.className = 'email-feedback is-error';
        feedback.textContent = 'Error de conexión al enviar el correo.';
      } finally {
